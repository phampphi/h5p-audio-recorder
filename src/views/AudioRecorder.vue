<template>
  <div class="h5p-audio-recorder-view">
    ...
    <timer ref="timer" v-bind:stopped="timerStopped" 
      v-bind:style="{visibility: timerVisible ? 'visible' : 'hidden'}"></timer>
...
<script>
  ...
  const TimerStates = [State.READY, State.RECORDING, State.PAUSED, State.PRE_RECORDING];
  ...
  export default {
    methods: {
      record: function() {
        ...
        if (this.timeLimit > 0)
          this.timeouts.push(setTimeout(() => this.done(), (this.timeLimit * 1000) + 200));
      },

      recordDelay: function() {
        this.$refs.timer && this.$refs.timer.reset(this.delayStart); // countdown
        this.state = State.PRE_RECORDING;
        const t = setTimeout(() => {
          if (this.state === State.PRE_RECORDING){ // make sure state is not changed
      ...
      autoRecord: function() {
        // this.state = State.READY; // ensure timer 'stopped' is changed
        (!this.isEditor && this.delayStart > 0) ? this.recordDelay() : this.record();
      },
      ...
      playAudioFile: function(){
        if (this.$refs.audioFilePlayer){
          this.timeouts.push(setTimeout(() => {
            ...
            this.$refs.audioFilePlayer.play();
          }, 2000));
        }
      },
    ...
    computed: {
      timerStopped: function(){
        return this.state !== State.RECORDING && this.state !== State.PRE_RECORDING;
      },
      timerVisible: function(){
        return !!TimerStates[this.state] ? 'visible' : 'hidden';
      }
    },
